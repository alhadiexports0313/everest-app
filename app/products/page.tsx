      ...
      <div className="lg:hidden fixed bottom-5 left-4 right-4 z-40">
        <div className="mx-auto flex max-w-[520px] flex-col gap-2 rounded-2xl border border-stone-200/60 bg-white/95 p-3 shadow-premium backdrop-blur">
          ...
          <button
            ...
            disabled={emailSubmitting}
            aria-busy={emailSubmitting}
            className="inline-flex items-center justify-center gap-2 rounded-full border border-stone-200 bg-white px-5 py-3 text-sm font-semibold text-stone-700 shadow-soft disabled:cursor-not-allowed disabled:opacity-60"
          >
            <Mail className="w-4 h-4" />
            {emailSubmitting
              ? isUrdu
          ...
          {emailError ? (
            <div className={`px-2 text-[11px] text-rose-500 ${isUrdu ? "text-right" : "text-left"}`}>
              {emailError}
            </div>
          ) : null}
        </div>
